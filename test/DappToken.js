...
contract('DappToken', function(accounts){
    ...
    it ('approves tokens for delegated transfer' , function(){
        ...
        return DappToken.deployed().then(function(instance){
            ...
            return tokenInstance.approve(accounts[1],100 ,{ from : accounts[0] });//create a transation 
        }).then(function(receipt){
            assert.equal(receipt.logs.length,1,'triggers one event');
            assert.equal(receipt.logs[0].event, 'Approval','should be the "Approval" event ');
            assert.equal(receipt.logs[0].args._owner, accounts[0],'logs the account the tokens are tansfered from');
...
it('handles delegated token transfers', function() {
    ...
    }).then(function(receipt) { //transfer event
      assert.equal(receipt.logs.length, 1, 'triggers one event');
      assert.equal(receipt.logs[0].event, 'Transfer', 'should be the "Transfer" event');
